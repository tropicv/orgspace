...
(function($)
{
	...
	$.Redactor.opts = {
			...
			buttons: ['html', 'formatting', 'sUper', 'sUb', 'bold', 'italic', 'deleted', 'underline', 'unorderedlist', 'orderedlist',
					  'outdent', 'indent',  'table', 'link', 'alignment', '|',
					  'horizontalrule', 'br', 'hlp'], // 'image', 'video', 'file', 'underline', 'alignleft', 'aligncenter', 'alignright', 'justify'
			buttonsHideOnMobile: [],
			...
			alignmentTags: ['P', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'DD', 'DL', 'DT', 'DIV', 'TD',
								...
								'HEADER', 'FOOTER', 'ASIDE', 'ARTICLE', 'LI', 'UL', 'OL'],
            ...
            ru: {
                    ...
                    table: 'Таблица',
                    link: 'Ссылка',
                    int_link: 'Вставить внутреннюю ссылку ...',
                    ...
                    br: 'br',
                    hlp: 'Всплывающая подсказка',
                    hlp_insert: 'Вставить подсказку ...',
                    unhlp: 'Удалить подсказку',
	...
	Redactor.fn = $.Redactor.prototype = {
		...
		exec: function(cmd, param, sync)
		{
            ...
            if (cmd === 'inserthtml' && this.bro() == 'Chrome')
            {
                this.execPasteFrag(param);
            }
            else if (cmd === 'inserthtml' && this.browser('chrome'))
            {
                this.execPasteFrag(param);
            }
            else if (cmd === 'inserthtml' && this.browser('msie'))
			{
				if (!this.isIe11())
				{
					this.focusWithSaveScroll();
		...
		execCommand: function(cmd, param, sync)
		{
            ...
            if (cmd === 'unhlp') return this.execUnhlp(cmd, param);
        ...
        execUnhlp: function(cmd, param)
		{
            this.bufferSet();
            
            var hlp = this.currentOrParentIs('A');
            if (hlp)
            {
                $(hlp).find('label').remove();
                $(hlp).replaceWith($(hlp).text());
		...
		cleanUnverified: function()
		{
			...
			if ($.inArray(this.$editor.find('ul, ol, li').tagName, this.opts.alignmentTags) !== -1)
			{
				this.$editor.find('ul, ol, li').removeAttr('style');
			}
		},
        ...
        inserthlp: function ()
        {
            ...
            if (this.getSelection().toString() != '') {
                var callback = $.proxy(function()
                {
                    ...
                    {
                        helptext = $(elem).find('label').text();
                        $(elem).find('label').remove();
                        text = $(elem).text();
                        

                        this.insert_help_node = elem;
                    }
                    else text = sel.toString();

                    $('#redactor_insert_text_area').val(helptext);
                    
                    $('#redactor_insert_word_area').val(text);
                    

                    $('#redactor_insert_hlp_btn').click($.proxy(this.helpProcess, this));
                ...
                this.modalInit(this.opts.curLang.hlp, this.opts.modal_help, 460, callback);
            } else {
		...
		{
			var helptext = $('#redactor_insert_text_area').val();
            var text = '';
            if (helptext != '') {
                text = $('#redactor_insert_word_area').val();
                var hlpllb = '<label class="hlplable" style="display:none;">'+helptext+'</label>';
                this.helpInsert('<a class="hlpllb" href="javascript:void(0);" onclick="return false" onmouseover="$(this).find(\'label\').show()" onmouseleave="$(this).find(\'label\').hide()">' + text + hlpllb+'</a>', $.trim(text), helptext);
            } else {
                alert('Текст подсказки не должен быть пустым');
            }

		},
        helpInsert: function (a, text, helptext)
		{
			this.selectionRestore();
            if (helptext !== '') {
                helptext = '<label class="hlplable" style="display:none;">'+helptext+'</label>';
            }
			if (text !== '')
			{
				if (this.insert_help_node)
				{
					this.bufferSet();

					$(this.insert_help_node).text(text).attr('onmouseover', '$(this).find(\'label\').show()');
					$(this.insert_help_node).text(text).attr('onmouseleave', '$(this).find(\'label\').hide()');
        ...
        intlink: function()
		{
			...
			var callback = $.proxy(function()
			{
                ...
                $('#suggest').keyup($.proxy(this.linksuggest, this));
			...
			this.modalInit(this.opts.curLang.link, this.opts.modal_intlink, 460, callback);
        ...
        linksuggest: function()
		{
            ...
            $.ajax({
                    type: 'post',
                    url: apppath+'article-intlink',
                    data: 'q='+$('#suggest').val(),
                    success: function(data){
                        $.each(data, function(i, group){
                            if (group['data'].length > 0) {
                                $('#sWrap').empty().show();
                                $.each(group['data'],function(){
                                    $('#sWrap').append('<div class="sItem" id="item_'+this.ispublic+'_('+this.chapter+')_'+this.id+'" onclick="_self.makelink($(this))">'+this.name+'</div>');
                                });
                            } else {
                                $('#sWrap').hide();
                            }
		...
		{
            var pub = parseInt(obj.attr('id').substring(obj.attr('id').indexOf('_')+1,obj.attr('id').indexOf('_(')));
            var chap = parseInt(obj.attr('id').substring(obj.attr('id').indexOf('(')+1,obj.attr('id').indexOf(')_')));
            var href = '';
            if (pub === 1) href += '/Народная_энциклопедия';
            if (chap === 1) href+= '/Глоссарий';
            if (chap === 2) href+= '/Водные_объекты';
            if (chap === 4) href+= '/О_воде_официально';
            href+='/'+obj.text();
            href = href.replace(/\s+/g,'_');
            $('#suggest').val(href);
            $('#redactor_link_url').val(href);
            $('#sWrap').hide();
            $.proxy(this.linkProcess, this);
        },
		// LINK
		linkShow: function()
		{
		...
		modalTemplatesInit: function()
		{
			$.extend( this.opts,
			{
                ...
                modal_intlink: String()
				+ '<section>'
					+ '<form id="redactorInsertLinkForm" method="post" action="">'
						+ '<input type="hidden" id="redactor_tab_selected" value="1" />'
						+ '<div class="redactor_tab" id="redactor_tab1">'
							+ '<label>Название статьи</label>'
							+ '<input type="text" id="suggest" class="redactor_input"  />'
							+ '<div id="sWrap" class="sWrap"/>'
							+ '<input type="text" id="redactor_link_url" class="redactor_input hide"  />'
							+ '<label>' + this.opts.curLang.text + '</label>'
				...
				+ '<section>'
                    + '<input type="hidden" id="redactor_tab_selected" value="1" />'
                    + '<div class="redactor_tab" id="redactor_tab">'
                        + '<label>' + this.opts.curLang.hlp + '</label>'
                        + '<textarea id="redactor_insert_text_area" style="width: 99%; height: 160px;"></textarea>'
                        + '<label>Слово</label>'
                        + '<input type="text" class="redactor_input redactor_link_text" id="redactor_insert_word_area" />'
                    + '</div>'
				+ '</section>'
				+ '<footer>'
					+ '<button class="redactor_modal_btn redactor_btn_modal_close">' + this.opts.curLang.cancel + '</button>'
					+ '<button id="redactor_insert_hlp_btn" class="redactor_modal_btn redactor_modal_action_btn">' + this.opts.curLang.insert + '</button>'
				+ '</footer>',
				...
				if (convertLinks && html && html.match(url))
				{
					...
					for (var i in matches)
					{
						...
						/*
							...
							See 
							//gotofritz.net/blog/code-snippets/escaping-in-replace-strings-in-javascript/
						*/
