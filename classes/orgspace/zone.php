...
class zone_db extends basic_db {
  function load_class(){
      parent::load_class();
  ...
  function search($keywords){
    $this->query("select *  from zone where ".$this->LIKE("description",$keywords)." or ".$this->LIKE("name",$keywords)." or ".$this->LIKE("ex_description",$keywords)." or ".$this->LIKE("slogan",$keywords)."order by name;");   
  }
...
class zone_editor extends basic_editor {   
}

class zone extends basic_manager {
  ...
  function add($state){
     $se=new zone_editor($this);
     return $se->add($state,new zone_db());
  }

  function edit($state){
     $se=new zone_editor($this);
     return $se->edit($state,new zone_db());
  }

  function del($state){
     $se=new zone_editor($this);
     return $se->del($state,new zone_db());
  }
  ...
  function view($state){ 
     ...
     $a=new imagegal_db();
     $a->list_album("zone", $s->getf("id"), ALBUM);	  
     d_open_list();
     while($a->lst()){
       d_put($a);
     }
