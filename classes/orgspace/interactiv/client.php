...
class client extends basic_manager {
  ...
  function add($state){
    set_title("Регистрация для закрытого раздела");  
    ...
    switch ($state){
      ...
      case SAVE_STATE:
       ...
       $res="<html><body><h3>Форма для регистрации в закрытом разделе</h3><br>";
       ...
       form_to_db("Регистрация для закрытого раздела",$res);

       mail_msg_html(get_proper("client_mail_to"),"Регистрация для закрытого раздела с www.orgspace.ru",$res);
