...
class ordtest extends basic_manager {
  ...
  function add($state){
    set_title("Заказать тест-драйв кресла Giroflex");	
    ...
    switch ($state){
      ...
      case SAVE_STATE:
       ...
       $res="<html><body><h3>Форма для заказа тест-драйва кресла Giroflex</h3><br>";
       ...
       $res.="<b>Тип кресла/b>: " .$f->getf("ctype")."<br>";
       ...
       form_to_db("заказ тест-драйва кресла Giroflex",$res);

       mail_msg_html(get_proper("ordtest_mail_to"),"Новый заказ тест-драйва кресла Giroflex с www.orgspace.ru",$res);
