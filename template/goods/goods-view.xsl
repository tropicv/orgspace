...
<xsl:if test="$admin=1">
[<a href="goods-edit?obj={goods_db/@id}">Редактировать предмет</a>]
[<a href="goods-del?obj={goods_db/@id}">Удалить предмет</a>]
[<a href="container-managewrapper?i=goods&amp;m=view&amp;obj={goods_db/@id}&amp;creator=container.view.goodsl4" target="blank">Редактировать шаблон</a>]
</xsl:if>
...
<!-- <h2><xsl:value-of select="goods_db/@name"/>: дополнительная информация</h2>
