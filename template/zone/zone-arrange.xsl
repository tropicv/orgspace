...
<xsl:template match="zone_arrange"> 
...
[<a href="container-managewrapper?i=zone&amp;m=arrange&amp;walias={zone_db/@walias}&amp;creator=container.view.goodsl4" target="blank">Редактировать шаблон</a>]
</xsl:if>


</xsl:template>

<xsl:template match="zone_arrange" mode="title"> 
<a class="white" href="zone-view?walias={zone_db/@walias}"><xsl:value-of select="zone_db/@short_name"/></a>
</xsl:template>

<xsl:template match="zone_arrange" mode="path"> 
...
<xsl:value-of select="$lc/zone/arrange"/>
</div>

</xsl:template>

<xsl:template match="zone_arrange" mode="left_start"> 
...
<xsl:template match="zone_arrange" mode="left_end"> 
</xsl:template>

<xsl:template match="zone_arrange" mode="content_title"> 
 <xsl:value-of select="zone_db/@name"/>
</xsl:template>

<xsl:template match="zone_arrange" mode="content_start"> 
</xsl:template>

<xsl:template match="zone_arrange" mode="content_end"> 
</xsl:template>

<xsl:template match="zone_arrange" mode="right_start"> 
...
<xsl:template match="zone_arrange" mode="right_end"> 
</xsl:template>

<xsl:template match="zone_arrange" mode="under_start"> 
</xsl:template>

<xsl:template match="zone_arrange" mode="under_end"> 
</xsl:template>
