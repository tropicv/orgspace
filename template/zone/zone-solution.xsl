...
<xsl:template match="zone_solution"> 
...
[<a href="container-managewrapper?i=zone&amp;m=solution&amp;walias={zone_db/@walias}&amp;creator=container.view.goodsl4" target="blank">Редактировать шаблон</a>]
</xsl:if>


</xsl:template>

<xsl:template match="zone_solution" mode="title"> 
<a class="white" href="zone-view?walias={zone_db/@walias}"><xsl:value-of select="zone_db/@short_name"/></a>
</xsl:template>

<xsl:template match="zone_solution" mode="path"> 
...
<!--<xsl:value-of select="$lc/zone/solution"/>-->
</div>

</xsl:template>

<xsl:template match="zone_solution" mode="left_start"> 
...
<xsl:template match="zone_solution" mode="left_end"> 
</xsl:template>

<xsl:template match="zone_solution" mode="content_title"> 
<xsl:value-of select="zone_db/@name"/>
</xsl:template>

<xsl:template match="zone_solution" mode="content_start"> 
</xsl:template>

<xsl:template match="zone_solution" mode="content_end"> 
</xsl:template>

<xsl:template match="zone_solution" mode="right_start"> 
<!--
<xsl:call-template name="zonermenu">
       <xsl:with-param name="cur" select='string("solution")'/>
...
<td class="wtext">
	...
	<xsl:choose>
	    <xsl:when test="//zone_solution/@walias=@walias">
		<div id="orgmenu14head" class="rmenu"><xsl:value-of select="@name" /></div>
	    </xsl:when>
	    <xsl:otherwise>
		<div id="orgmenu14head" class="rmenu"><a href="zone-solution?walias={@walias}"><xsl:value-of select="@name" /></a></div>
	    </xsl:otherwise>
	</xsl:choose>
    </xsl:for-each>
...
<xsl:template match="zone_solution" mode="right_end"> 
</xsl:template>

<xsl:template match="zone_solution" mode="under_start"> 
</xsl:template>

<xsl:template match="zone_solution" mode="under_end"> 
</xsl:template>
