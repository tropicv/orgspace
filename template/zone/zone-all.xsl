...
<td class="wtext" width="500" rowspan="2">
  <xsl:value-of select="//botm_link/container_view/*/container_db[@layer=0]/element_db[@layer=1]/*/*/@title" disable-output-escaping="yes"/>
</td>
...
<table border="0" cellspacing="0" cellpadding="0" class="any">
 <tr>
  <td class="abswtitle" nowrap="nowrap" valign="top"><xsl:value-of select="$lc/zone/@name"/>:</td>
  <td class="wtext" nowrap="nowrap" valign="top">
<xsl:variable name="num">
<xsl:choose>
<xsl:when test="count(list/zone_db)=4">2</xsl:when>
<xsl:otherwise>4</xsl:otherwise>
</xsl:choose>
</xsl:variable>
   <xsl:for-each select="list/zone_db">
     <a href="zone-view?walias={@walias}"><xsl:value-of select="@short_name"/></a><br/>
...
<a href="zone-view?walias={@walias}">
   <xsl:call-template name="res_image_stuff">
          <xsl:with-param name="id" select="concat('zone_',concat(@id,'_2'))"/>
...
<tr>
<td colspan="2">
<a href="zone-solution?walias={@walias}">
   <xsl:call-template name="res_image_stuff">
          <xsl:with-param name="id" select="concat('zone_',concat(@id,'_1'))"/>
...
<a href="zone-solution?walias={@walias}"><xsl:value-of select="@name"/></a><br/><br/>
...
<xsl:if test="$admin=1">
<br/><div class="zone_action">[<a href="zone-del?obj={@id}">Удалить зону</a>]</div>
<br/>
</xsl:if>
...
<tr class="link">
<td><a href="zone-view?walias={@walias}"><xsl:value-of select="@name"/></a></td>
<td align="right"><a href="zone-view?walias={@walias}"><img src="/images/cat-arr.gif" width="6" height="9" border="0" align="absmiddle"/></a><br/></td>
</tr>
...
<xsl:if test="$admin=1">
[<a href="zone-add">Добавить</a>]<br/>
[<a href="container-managewrapper?i=zone&amp;m=all&amp;creator=container.view.empty" target="blank">Редактировать шаблон</a>]
</xsl:if>
