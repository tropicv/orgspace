...
<xsl:template match="ordtest_add"> 
...
[<a href="container-managewrapper?i=ordtest&amp;m=add&amp;state={@last_state}&amp;creator=container.view.dbuyl4" target="blank">Редактировать шаблон</a>]
</xsl:when>
<xsl:otherwise>
[<a href="container-managewrapper?i=ordtest&amp;m=add&amp;creator=container.view.dbuyl4" target="blank">Редактировать шаблон</a>]
</xsl:otherwise>
</xsl:choose>
</xsl:if>
...
<xsl:template match="ordtest_add" mode="title"> 

<xsl:if test="not($wrapper/container_db/element_db[@layer=5])">
<xsl:value-of select="$lc/ordtest/alt"/>
</xsl:if>
</xsl:template>

<xsl:template match="ordtest_add" mode="path"> 
...
<xsl:template match="ordtest_add" mode="left_start"> 
</xsl:template>

<xsl:template match="ordtest_add" mode="left_end"> 
</xsl:template>

<xsl:template match="ordtest_add" mode="content_title"> 
<xsl:value-of select="$lc/ordtest/alt"/>
</xsl:template>

<xsl:template match="ordtest_add" mode="content_start"> 
</xsl:template>

<xsl:template match="ordtest_add" mode="content_end"> 

<xsl:if test="not($wrapper/container_db/element_db[@layer=2])">
<h1><xsl:value-of select="$lc/ordtest/@name"/></h1>
</xsl:if>
...
<p>* <xsl:value-of select="$lc/feedback/req"/></p>
 ...
 <form name="ordtest" id="ordtest" method="POST" action="ordtest-add?state=1" enctype="multipart/form-data">

<xsl:variable name="c" select="ordtest_db"/>
...
<tr>
<td class="input"><input type="text" name="tel" size="45" value="{$c/@mail}"/></td>
<td class="inputr"><img src="/images/dot.gif" width="5" height="15" border="0"/><br/></td>
</tr>
...
<tr>
<td colspan="2"><xsl:value-of select="$lc/ordtest/adr"/></td>
</tr>
<tr>
...
<tr>
<td colspan="2"><xsl:value-of select="$lc/ordtest/ctype"/></td>
</tr>
<tr>
<td class="input"><input type="text" name="ctype" size="45" value="{$c/@ctype}"/></td>
<td class="inputr"><img src="/images/dot.gif" width="5" height="15" border="0"/><br/></td>
</tr>
...
<tr>
<td colspan="2"><xsl:value-of select="$lc/ordtest/coment"/></td>
</tr>
...
<xsl:for-each select="$c/hear_type/word">
...
<!--
  ...
  <input onclick="reset_form('ordtest');return false;" type="image" src="/images/clear{$lc/@pref}.gif" width="68" height="14" border="0" align="absmiddle" alt="{$lc/clear}" title="{$lc/clear}"/>
-->
    <xsl:call-template name="form_button">
	<xsl:with-param name="name_form">ordtest</xsl:with-param>
    </xsl:call-template>
...
<xsl:template match="ordtest_add" mode="right_start"> 
<xsl:if test="not($wrapper/container_db/element_db[@layer=3])">
<xsl:value-of select="$lc/ordtest/@name"/>
</xsl:if>
</xsl:template>

<xsl:template match="ordtest_add" mode="right_end"> 
</xsl:template>

<xsl:template match="ordtest_add" mode="right_print"> 
</xsl:template>

<xsl:template match="ordtest_add" mode="under_start"> 
</xsl:template>

<xsl:template match="ordtest_add" mode="under_end"> 
</xsl:template>
