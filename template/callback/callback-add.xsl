...
<xsl:template match="callback_add"> 
...
[<a href="container-managewrapper?i=callback&amp;m=add&amp;state={@last_state}&amp;creator=container.view.dbuyl4" target="blank">Редактировать шаблон</a>]
</xsl:when>
<xsl:otherwise>
[<a href="container-managewrapper?i=callback&amp;m=add&amp;creator=container.view.dbuyl4" target="blank">Редактировать шаблон</a>]
</xsl:otherwise>
</xsl:choose>
</xsl:if>
...
<xsl:template match="callback_add" mode="title"> 

<xsl:if test="not($wrapper/container_db/element_db[@layer=5])">
<xsl:value-of select="$lc/callback/alt"/>
</xsl:if>
</xsl:template>

<xsl:template match="callback_add" mode="path"> 
...
<xsl:template match="callback_add" mode="left_start"> 
</xsl:template>

<xsl:template match="callback_add" mode="left_end">
</xsl:template>

<xsl:template match="callback_add" mode="content_title">  
<xsl:value-of select="$lc/callback/alt"/>
</xsl:template>

<xsl:template match="callback_add" mode="content_start"> 
</xsl:template>

<xsl:template match="callback_add" mode="content_end"> 

<xsl:if test="not($wrapper/container_db/element_db[@layer=2])">
<h1><xsl:value-of select="$lc/callback/@name"/></h1>
</xsl:if>
...
<p>* <xsl:value-of select="$lc/feedback/req"/></p>
 ...
 <form name="callback" id="callback" method="POST" action="callback-add?state=1" enctype="multipart/form-data">
...
<tr>
<td colspan="2"><xsl:value-of select="$lc/callback/time"/></td>
</tr>
<tr>
...
<!--
  ...
  <input onclick="reset_form('callback');return false;" type="image" src="/images/clear{$lc/@pref}.gif" width="68" height="14" border="0" align="absmiddle" alt="{$lc/clear}" title="{$lc/clear}"/>
-->
    <xsl:call-template name="form_button">
	<xsl:with-param name="name_form">callback</xsl:with-param>
    </xsl:call-template>
</td>
</tr>
...
<xsl:template match="callback_add" mode="right_start"> 
<xsl:if test="not($wrapper/container_db/element_db[@layer=3])">
<xsl:value-of select="$lc/callback/@name"/>
</xsl:if>
</xsl:template>

<xsl:template match="callback_add" mode="right_end"> 
</xsl:template>

<xsl:template match="callback_add" mode="right_print"> 
</xsl:template>

<xsl:template match="callback_add" mode="under_start"> 
</xsl:template>

<xsl:template match="callback_add" mode="under_end"> 
</xsl:template>
