...
[<a href="container-managewrapper?i=ordcat&amp;m=add&amp;state={@last_state}&amp;creator=container.view.dbuyl4" target="blank">Редактировать шаблон</a>]
</xsl:when>
<xsl:otherwise>
[<a href="container-managewrapper?i=ordcat&amp;m=add&amp;creator=container.view.dbuyl4" target="blank">Редактировать шаблон</a>]
</xsl:otherwise>
</xsl:choose>
</xsl:if>
...
<xsl:template match="ordcat_add" mode="title"> 

<xsl:if test="not($wrapper/container_db/element_db[@layer=5])">
<xsl:value-of select="$lc/ordcat/alt"/>
</xsl:if>
</xsl:template>

<xsl:template match="ordcat_add" mode="path"> 
...
<xsl:template match="ordcat_add" mode="left_start"> 
</xsl:template>

<xsl:template match="ordcat_add" mode="left_end"> 
</xsl:template>

<xsl:template match="ordcat_add" mode="content_title"> 
<xsl:value-of select="$lc/ordcat/alt"/>
</xsl:template>

<xsl:template match="ordcat_add" mode="content_start"> 
</xsl:template>

<xsl:template match="ordcat_add" mode="content_end"> 

<xsl:if test="not($wrapper/container_db/element_db[@layer=2])">
<h1><xsl:value-of select="$lc/ordcat/@name"/></h1>
</xsl:if>
...
<xsl:template match="ordcat_add" mode="right_start"> 
<xsl:if test="not($wrapper/container_db/element_db[@layer=3])">
<xsl:value-of select="$lc/ordcat/@name"/>
</xsl:if>
</xsl:template>

<xsl:template match="ordcat_add" mode="right_end"> 
</xsl:template>

<xsl:template match="ordcat_add" mode="right_print"> 
</xsl:template>

<xsl:template match="ordcat_add" mode="under_start"> 
</xsl:template>

<xsl:template match="ordcat_add" mode="under_end"> 
</xsl:template>
