...
[<a href="container-managewrapper?i=ordpr&amp;m=add&amp;state={@last_state}&amp;creator=container.view.dbuyl4" target="blank">Редактировать шаблон</a>]
</xsl:when>
<xsl:otherwise>
[<a href="container-managewrapper?i=ordpr&amp;m=add&amp;creator=container.view.dbuyl4" target="blank">Редактировать шаблон</a>]
</xsl:otherwise>
</xsl:choose>
</xsl:if>
...
<xsl:template match="ordpr_add" mode="title"> 

<xsl:if test="not($wrapper/container_db/element_db[@layer=5])">
<xsl:value-of select="$lc/ordpr/alt"/>
</xsl:if>
</xsl:template>

<xsl:template match="ordpr_add" mode="path"> 
...
<xsl:template match="ordpr_add" mode="left_start"> 
</xsl:template>

<xsl:template match="ordpr_add" mode="left_end"> 
</xsl:template>

<xsl:template match="ordpr_add" mode="content_title"> 
<xsl:value-of select="$lc/ordpr/alt"/>
</xsl:template>

<xsl:template match="ordpr_add" mode="content_start"> 
</xsl:template>

<xsl:template match="ordpr_add" mode="content_end"> 

<xsl:if test="not($wrapper/container_db/element_db[@layer=2])">
<h1><xsl:value-of select="$lc/ordpr/@name"/></h1>
</xsl:if>
...
<p>* <xsl:value-of select="$lc/feedback/req"/></p>
 ...
 <form name="ordpr" id="ordpr" method="POST" action="ordpr-add?state=1" enctype="multipart/form-data">

<xsl:variable name="c" select="ordpr_db"/>
...
<tr>
<td colspan="2"><xsl:value-of select="$lc/ordpr/adr"/></td>
</tr>
<tr>
...
<tr>
<td colspan="2"><xsl:value-of select="$lc/ordpr/time"/></td>
</tr>
<tr>
...
<tr>
<td colspan="2"><xsl:value-of select="$lc/ordpr/coment"/></td>
</tr>
...
<!--
  ...
  <input onclick="reset_form('ordpr');return false;" type="image" src="/images/clear{$lc/@pref}.gif" width="68" height="14" border="0" align="absmiddle" alt="{$lc/clear}" title="{$lc/clear}"/>
-->
    <xsl:call-template name="form_button">
	<xsl:with-param name="name_form">ordpr</xsl:with-param>
    </xsl:call-template>
</td>
</tr>
...
<xsl:template match="ordpr_add" mode="right_start"> 
<xsl:if test="not($wrapper/container_db/element_db[@layer=3])">
<xsl:value-of select="$lc/ordpr/@name"/>
</xsl:if>
</xsl:template>

<xsl:template match="ordpr_add" mode="right_end"> 
</xsl:template>

<xsl:template match="ordpr_add" mode="right_print"> 
</xsl:template>

<xsl:template match="ordpr_add" mode="under_start"> 
</xsl:template>

<xsl:template match="ordpr_add" mode="under_end"> 
</xsl:template>
