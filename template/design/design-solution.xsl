...
<xsl:template match="design_solution"> 
...
[<a href="container-managewrapper?i=design&amp;m=solution&amp;walias={design_db/@walias}&amp;creator=container.view.goodsl4" target="blank">Редактировать шаблон</a>]
</xsl:if>


</xsl:template>

<xsl:template match="design_solution" mode="title"> 
<a class="white" href="design-view?walias={design_db/@walias}"><xsl:value-of select="design_db/@short_name"/></a>
</xsl:template>

<xsl:template match="design_solution" mode="path"> 
...
<xsl:value-of select="$lc/design/solution"/>
</div>

</xsl:template>

<xsl:template match="design_solution" mode="left_start"> 
...
<xsl:template match="design_solution" mode="left_end"> 
</xsl:template>

<xsl:template match="design_solution" mode="content_title"> 
<xsl:value-of select="design_db/@name"/>
</xsl:template>

<xsl:template match="design_solution" mode="content_start"> 
</xsl:template>

<xsl:template match="design_solution" mode="content_end"> 
</xsl:template>

<xsl:template match="design_solution" mode="right_start"> 
...
<td class="wtext">
 <div name="orgmenu11head" id="orgmenu11head">
  <a onMouseOut="Turn('11',0);" onMouseOver="Turn('11',1);" href="design-all">
   <xsl:value-of select="$lc/design/other"/>
  </a>
 </div>
</td>
</tr>
...
<xsl:template match="design_solution" mode="right_end"> 
</xsl:template>

<xsl:template match="design_solution" mode="under_start"> 
</xsl:template>

<xsl:template match="design_solution" mode="under_end"> 
</xsl:template>
