...
<td class="wtext" width="500" rowspan="2">
  <xsl:value-of select="//botm_link/container_view/*/container_db[@layer=0]/element_db[@layer=2]/*/*/@title" disable-output-escaping="yes"/>
</td>
...
<table border="0" cellspacing="0" cellpadding="0" class="any">
 <tr>
  <td class="abswtitle" nowrap="nowrap" valign="top"><xsl:value-of select="$lc/serial/@name"/>:</td>
  <td class="wtext" nowrap="nowrap" valign="top">
<xsl:variable name="num">
<xsl:choose>
<xsl:when test="count(list/serial_db)=4">2</xsl:when>
<xsl:otherwise>4</xsl:otherwise>
</xsl:choose>
</xsl:variable>
   <xsl:for-each select="list/serial_db[@srt!=1000]"> <!-- srt=1000 - archivnai seria -->
     <!--<a href="serial-view?walias={@walias}"><xsl:value-of select="@short_name"/></a><br/>-->
     <a href="serial-addinf?walias={@walias}"><xsl:value-of select="@short_name"/></a><br/>
       <xsl:if test="(position() mod $num = 0) and (position()!=last())">
        <xsl:text disable-output-escaping="yes">&lt;/td>&lt;td class="wtext" valign="top"></xsl:text>
       </xsl:if>
   </xsl:for-each>
  </td>
 </tr>
...
<xsl:for-each select="list/serial_db">
<xsl:if test='@sh_tmb!=1'>
<a href="serial-addinf?walias={@walias}">
   <xsl:call-template name="res_image_stuff">
          <xsl:with-param name="id" select="concat('serial_',concat(@id,'_2'))"/>
...
<a href="serial-addinf?walias={@walias}">
   <xsl:call-template name="res_image_stuff">
          <xsl:with-param name="id" select="concat('serial_',concat(@id,'_1'))"/>
...
<a href="serial-addinf?walias={@walias}"><xsl:value-of select="@name"/></a><br/><br/>
...
<tr><td colspan="2">
<a href="serial-view?walias={@walias}">
   <xsl:call-template name="res_image_stuff">
          <xsl:with-param name="id" select="concat('serial_',concat(@id,'_1'))"/>
          <xsl:with-param name="type" select="foto"/>
          <xsl:with-param name="max" select='string("158")'/>
...
<tr class="link">
<td><a href="serial-view?walias={@walias}"><xsl:value-of select="@name"/></a></td>
<td align="right"><a href="serial-view?walias={@walias}"><img src="/images/cat-arr.gif" width="6" height="9" border="0" align="absmiddle"/></a><br/></td>
</tr>
...
<xsl:if test="$admin=1">
[<a href="serial-add">Добавить</a>]<br/>
[<a href="container-managewrapper?i=serial&amp;m=all&amp;creator=container.view.empty" target="blank">Редактировать шаблон</a>]
</xsl:if>
<br/>
