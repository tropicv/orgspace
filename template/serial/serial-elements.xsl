...
<xsl:template match="serial_elements"> 
...
[<a href="container-managewrapper?i=serial&amp;m=elements&amp;walias={serial_db/@walias}&amp;creator=container.view.goodsl4" target="blank">Редактировать шаблон</a>]
</xsl:if>


</xsl:template>

<xsl:template match="serial_elements" mode="title"> 
<a class="white" href="serial-view?walias={serial_db/@walias}"><xsl:value-of select="serial_db/@short_name"/></a>
</xsl:template>

<xsl:template match="serial_elements" mode="path"> 
...
<xsl:value-of select="$lc/serial/elements"/>
</div>

</xsl:template>

<xsl:template match="serial_elements" mode="left_start"> 
...
<xsl:template match="serial_elements" mode="left_end"> 
</xsl:template>

<xsl:template match="serial_elements" mode="content_title"> 
 <xsl:value-of select="serial_db/@name"/>
</xsl:template>

<xsl:template match="serial_elements" mode="content_start"> 
</xsl:template>

<xsl:template match="serial_elements" mode="content_end"> 
</xsl:template>

<xsl:template match="serial_elements" mode="right_start"> 

<xsl:call-template name="serialrmenu">
       <xsl:with-param name="cur" select='string("elements")'/>
...
<xsl:template match="serial_elements" mode="right_end"> 
</xsl:template>

<xsl:template match="serial_elements" mode="under_start"> 
</xsl:template>

<xsl:template match="serial_elements" mode="under_end"> 
</xsl:template>
